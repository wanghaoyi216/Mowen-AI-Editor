"""MySQL 8.0 初始 schema（V3 迁移基线）

Revision ID: 20260604_0001_mysql_initial
Revises:
Create Date: 2026-06-04

说明：
- v2-overhaul 中间件重构：从 PostgreSQL/pgvector 迁移到 MySQL 8.0。
- 本迁移对应此前所有 PG 迁移的合并基线（initial_schema + chapter_versions +
  workflow_execution_id + task_logs_step_no + task_checkpoints），方便在新
  MySQL 库上一次性建立完整 schema。
- 字符集：utf8mb4 / utf8mb4_unicode_ci（通过 mysql_charset / mysql_collate）。
- 移除了 PG 特有语法：CREATE EXTENSION vector、USING gist、ARRAY[]、JSONB 等。
- content_embeddings.embedding 改为 JSON（list[float]），不再使用 Vector(1536)。
- 现有 PG 迁移版本（20260531_0001, 20260531_0002, 20260601_0003, 20260601_0004,
  20260602_0004, 20260603_0001）保留在 versions/ 目录中作为历史档案。
"""
...
revision: str = "20260604_0001_mysql_initial"
down_revision: Union[str, Sequence[str], None] = None
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None


# 公共选项：所有表使用 utf8mb4 + utf8mb4_unicode_ci
_MYSQL_TABLE_KW = {"mysql_charset": "utf8mb4", "mysql_collate": "utf8mb4_unicode_ci"}
_MYSQL_COL_KW = {"mysql_charset": "utf8mb4", "mysql_collate": "utf8mb4_unicode_ci"}
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column(
            "project_id", sa.Integer(), sa.ForeignKey("novel_projects.id"), nullable=False
        ),
        sa.Column("name", sa.String(length=200), nullable=False),
        ...
        # MySQL 迁移：personality_vector 由 JSONB 改为 JSON
    ...
    op.create_table(
        ...
        sa.Column(
            "plot_line_id", sa.Integer(), sa.ForeignKey("plot_lines.id"), nullable=True
        ),
        sa.Column("title", sa.String(length=200), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column(
            "chapter_id", sa.Integer(), sa.ForeignKey("chapters.id"), nullable=True
        ),
        sa.Column("title", sa.String(length=200), nullable=False),
    ...
    op.create_index(
        "ix_character_relationships_id", "character_relationships", ["id"]
    )
    op.create_index(
        "ix_character_relationships_project_id",
        "character_relationships",
        ["project_id"],
    )
    op.create_index(
        "ix_character_relationships_source_character_id",
        "character_relationships",
        ["source_character_id"],
    )
    op.create_index(
        "ix_character_relationships_target_character_id",
        "character_relationships",
        ["target_character_id"],
    )
    op.create_index(
        "ix_character_relationships_relation_type",
        "character_relationships",
        ["relation_type"],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "event_id", sa.Integer(), sa.ForeignKey("story_events.id"), nullable=False
        ),
    ...
    op.create_index(
        "ix_character_event_participations_id", "character_event_participations", ["id"]
    )
    op.create_index(
        "ix_character_event_participations_project_id",
        "character_event_participations",
        ["project_id"],
    )
    op.create_index(
        "ix_character_event_participations_character_id",
        "character_event_participations",
        ["character_id"],
    )
    op.create_index(
        "ix_character_event_participations_event_id",
        "character_event_participations",
        ["event_id"],
    )
    ...
    op.create_index(
        "ix_worldbook_entries_project_id", "worldbook_entries", ["project_id"]
    )
    ...
    op.create_index(
        "ix_trend_explorations_project_id", "trend_explorations", ["project_id"]
    )

    # ------------------------------------------------------------------
    # ai_tasks（含 workflow_execution_id 扩展列）
    # ------------------------------------------------------------------
    op.create_table(
        "ai_tasks",
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        ...
        sa.Column("workflow_execution_id", sa.String(length=120), nullable=True),
        sa.Column("mode", sa.String(length=20), server_default="confirm", nullable=True),
        sa.Column("current_workflow_id", sa.String(length=50), nullable=True),
        sa.Column("current_confirmation_point", sa.String(length=50), nullable=True),
    ...
    op.create_index(
        "ix_ai_tasks_workflow_execution_id",
        "ai_tasks",
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column(
            "task_id", sa.Integer(), sa.ForeignKey("ai_tasks.id"), nullable=False
        ),
    ...
    op.create_index(
        "ix_task_checkpoints_project_id", "task_checkpoints", ["project_id"]
    )

    # ------------------------------------------------------------------
    # confirmation_requests（summary 由 JSONB 改为 JSON）
    # ------------------------------------------------------------------
    op.create_table(
    ...
    # ------------------------------------------------------------------
    # content_embeddings（embedding 由 Vector(1536) 改为 JSON）
    # ------------------------------------------------------------------
    op.create_table(
        "content_embeddings",
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        ...
        # MySQL 不支持 pgvector，这里用 JSON 存 list[float]
    ...
    op.create_index(
        "ix_content_embeddings_project", "content_embeddings", ["project_id"]
    )
    ...
    op.create_index(
        "ix_chapter_versions_project_id", "chapter_versions", ["project_id"]
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    op.drop_index("ix_content_embeddings_type", table_name="content_embeddings")
    op.drop_index("ix_content_embeddings_project", table_name="content_embeddings")
    op.drop_table("content_embeddings")

    op.drop_index("ix_confirmation_status", table_name="confirmation_requests")
    op.drop_index("ix_confirmation_task_id", table_name="confirmation_requests")
    ...
    op.drop_index(
        "ix_character_event_participations_event_id",
        table_name="character_event_participations",
    )
    op.drop_index(
        "ix_character_event_participations_character_id",
        table_name="character_event_participations",
    ...
    op.drop_index(
        "ix_character_event_participations_id",
        table_name="character_event_participations",
    )
    ...
    op.drop_index("ix_characters_id", "characters", ["id"])
